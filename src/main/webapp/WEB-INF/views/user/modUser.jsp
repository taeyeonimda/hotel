...
function chnageUser(){ 
	$('#chnageBtn').click(()=>{
		...
		$.ajax({
			...
			success: ()=>{
				alert("수정에 성공하였습니다.");
				setTimeout('location.href="../user/userInfo"',1500);
			},
			error: ()=>{
				alert("수정에 실패했습니다."); 
			}
...
<form>
        ...
        <div class="form-group"><input class="form-control" id='userId' value = "${sessionScope.userInfo.userId}" readonly ></div>
		...
		<div class="form-group"><input class="form-control" id='userTel' value = "${sessionScope.userInfo.userTel}" ></div>
        <div class="form-group"><input class="form-control" id='userEmail' value = "${sessionScope.userInfo.userEmail}"></div>
		<input type='hidden' id='originPwd' value='${sessionScope.userInfo.userPwd}'>
		<div class="form-group row">
			<div class='col-sm-9'><input class="form-control" id ="userAddr"  name = 'userAddr' value='${sessionScope.userInfo.userAddr}'placeholder="주소"  readonly></div>
